...
public class QuizPane
    ...
    private void jbInit() {
        choicesPanel.setBorder(BorderFactory.createEmptyBorder(0, 20, 10, 10));
    ...
    void showAnswerButton_actionPerformed(ActionEvent e) {
        if (setupPane.isNoMultipleChoice()) {
            ++totalAsked;
            String dialogString = currentWord.getSide(!setupPane.isFlipped()) + "\n" +
    ...
    public Rectangle getMaxBounds(float fontSize) {
        ...
        while (it.hasNext()) {
            ...
            if (f != null) {
                ...
                if (rect.getWidth() > biggest.width)
                    biggest.width = (int) rect.getWidth();
                if (rect.getHeight() > biggest.height)
