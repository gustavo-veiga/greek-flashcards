...
public class EditorFrame extends JFrame {
  ...
  private void loadLesson(String fileName) {
    ...
    for (int i = 0; i < wordCount; i++) {
      words.add(new WordEntry(lesson.getProperty("word"+Integer.toString(i)), lesson.getProperty("answers"+Integer.toString(i))));
    }
    showLesson();
  }
  ...
  private void showLesson() {
    setTitle("FlashCards Editor (c) CrossWire Bible Society http://crosswire.org - " + lesson.getProperty("fileName"));
  ...
  void jButton1_actionPerformed(ActionEvent e) {
    ...
    if (dialog.showOpenDialog(this) == dialog.APPROVE_OPTION) {
      loadLesson(dialog.getSelectedFile().getName());
    }
  }
